...
public class MessageAdapter extends BaseAdapter {
	...
	public void removeHeadMsg() {
		AppLogger.i("before remove mMsgList.size() = " + mMsgList.size());
		...
		AppLogger.i("after remove mMsgList.size() = " + mMsgList.size());
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		//holder.head.setBackgroundResource(PushApplication.heads[item
				//.getHeadImg()]);
		holder.head.setBackgroundResource(R.drawable.honey_face);
	...
	private CharSequence convertNormalStringToSpannableString(String message) {
		...
		while (localMatcher.find()) {
			...
			if (m - k < 8) {
				if (GlobalContext.getInstance().getFaceMap()
						.containsKey(str2)) {
					int face = GlobalContext.getInstance().getFaceMap()
							.get(str2);
