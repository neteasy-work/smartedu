package com.engc.smartedu.othercomponent.smack;
...
import com.engc.smartedu.support.database.RosterProvider.RosterConstants;
...
import com.engc.smartedu.support.utils.StatusMode;
...
public class SmackImpl implements Smack {
	...
	private AppService mService;// 主服务
	...
	public SmackImpl(AppService service) {
	...
	@Override
	public boolean login(String account, String password) throws AppException {// 登陆实现
		try {
			if (mXMPPConnection.isConnected()) {// 首先判断是否还连接着服务器，需要先断开
				try {
					mXMPPConnection.disconnect();
				} catch (Exception e) {
					AppLogger.d("conn.disconnect() failed: " + e);
				}
			}
			...
			AppLogger.e(SmackImpl.class, "login(): " + Log.getStackTraceString(e));
			throw new AppException(e.getLocalizedMessage(), e.getCause());
		}
		...
		mPacketListener = new PacketListener() {
			public void processPacket(Packet packet) {
				try {
					...
					// processPacket :(
					AppLogger.e("failed to process packet:");
					e.printStackTrace();
				}
			}
		};
	...
	private class PongTimeoutAlarmReceiver extends BroadcastReceiver {
		public void onReceive(Context ctx, Intent i) {
			AppLogger.d("Ping: timeout for " + mPingID);
			mService.postConnectionFailed(AppService.PONG_TIMEOUT);
			logout();// 超时就断开连接
		}
	}
	...
	private class PingAlarmReceiver extends BroadcastReceiver {
		public void onReceive(Context ctx, Intent i) {
			if (mXMPPConnection.isAuthenticated()) {
				...
				AppLogger.d("Ping: alarm received, but not connected to server.");
		}
	}
	...
	private void registerRosterListener() {
		...
		mRosterListener = new RosterListener() {
			...
			public void presenceChanged(Presence presence) {// 联系人状态改变，比如在线或离开、隐身之类
				AppLogger.i("presenceChanged(" + presence.getFrom() + "): " + presence);
			...
			public void entriesUpdated(Collection<String> entries) {// 更新数据库，第一次登陆
				...
				AppLogger.i("entriesUpdated(" + entries + ")");
			...
			public void entriesDeleted(Collection<String> entries) {// 有好友删除时，
				AppLogger.i("entriesDeleted(" + entries + ")");
				for (String entry : entries) {
			...
			public void entriesAdded(Collection<String> entries) {// 有人添加好友时，我这里没有弹出对话框确认，直接添加到数据库
				AppLogger.i("entriesAdded(" + entries + ")");
	...
	private void addRosterEntryToDB(final RosterEntry entry) {
		...
		AppLogger.i("addRosterEntryToDB: Inserted " + uri);
	}
	...
	private void deleteRosterEntryFromDB(final String jabberID) {
		...
		AppLogger.i("deleteRosterEntryFromDB: Deleted " + count + " entries");
	}
	...
	private void initServiceDiscovery() {
		...
		dm.registerReceiptReceivedListener(new DeliveryReceiptManager.ReceiptReceivedListener() {
			public void onReceiptReceived(String fromJid, String toJid,
					String receiptId) {
				AppLogger.d(SmackImpl.class, "got delivery receipt for " + receiptId);
	...
	public void addRosterItem(String user, String alias, String group)
			throws AppException {// 添加联系人，供外部服务调用
	...
	@Override
	public void removeRosterItem(String user) throws AppException {// 删除联系人，供外部服务调用
		// TODO Auto-generated method stub
		AppLogger.d("removeRosterItem(" + user + ")");

		removeRosterEntry(user);
		mService.rosterChanged();
	}

	private void removeRosterEntry(String user) throws AppException {
		mRoster = mXMPPConnection.getRoster();
		try {
	...
	public void moveRosterItemToGroup(String user, String group)
			throws AppException {// 移动好友到其他分组，供外部服务调用
		// TODO Auto-generated method stub
	...
	private void removeRosterEntryFromGroups(RosterEntry rosterEntry)
			throws AppException {// 从对应组中删除联系人，供外部服务调用
	...
	public void renameRosterGroup(String group, String newGroup) {// 重命名分组
		// TODO Auto-generated method stub
		AppLogger.i("oldgroup=" + group + ", newgroup=" + newGroup);
		mRoster = mXMPPConnection.getRoster();
	...
	public boolean logout() {// 注销登录
		AppLogger.d("unRegisterCallback()");
		...
		if (mXMPPConnection.isConnected()) {
			...
			new Thread() {
				public void run() {
					AppLogger.d("shutDown thread started");
					mXMPPConnection.disconnect();
					AppLogger.d("shutDown thread finished");
				}
			}.start();
		}
//		setStatusOffline();
